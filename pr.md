...
각 렌더링해야될 템플릿을 만들고 구성하는걸까..? use client 같은 것을 보고 파싱하는 방식일까..

- Router 분리
  server client router를 두번씩 넣어주고 싶지 않다는 생각을 했다.
  짱돌을 아무리 굴려봐도 지금 이상황에서는 두번 작성할 수 밖에 없다.
  그럼 프레임워크에서는..? 프레임워크에서는 이것을 묶어놓은 동작을 만들어놓은걸까
